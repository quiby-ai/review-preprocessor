...
func (r *CleanRepository) UpsertBatch(ctx context.Context, items []CleanReview) error {
	...
	stmt, err := tx.PrepareContext(ctx, `
		INSERT INTO clean_reviews (id, app_id, country, rating, title, content_clean, language, reviewed_at, response_date, response_content_clean)
	...
	for _, it := range items {
		_, err := stmt.ExecContext(ctx, it.ID, it.AppID, it.Country, it.Rating, it.Title, it.ContentClean, it.Language, it.ReviewedAt, it.ResponseDate, it.ResponseContentClean)
		if err != nil {
			tx.Rollback()
			return err
		}
